...
//       <span className="text-gray-600 text-xs">Loading 3D...</span>
//     </div>
//   ),
// });

// const productImages = [
//   { id: "stealt", name: "Stealth", imageUrl: "/images/transparent.png", type: "image" ,price: "340"},
//   { id: "emerald", name: "Emerald", imageUrl: "/images/pixels.png", type: "image",price: "240" },
//   { id: "red", name: "Reddish", imageUrl: "/images/transparen2t.png", type: "image" ,price: "540"},
//   { id: "blue", name: "Blueish", imageUrl: "/images/transparent.png", type: "image"  ,price: "540"},
//   { id: "green", name: "Greeny", imageUrl: "/images/transparent.png", type: "image"  ,price: "540"},
//   { id: "yellow", name: "Yellow", imageUrl: "/images/transparent.png", type: "image" ,price: "540" },
// ];

// const ITEM_COUNT = productImages.length;
// const CONTAINER_SIZE = 600;
// const RADIUS = CONTAINER_SIZE / 2;
// const ITEM_WIDTH = 120;
// const ITEM_HEIGHT = 140;
// const ITEM_OFFSET = 20; // Reduced from 100 to bring items closer to circle
...
//   // Calculate rotation angle to always have selected item at 180 degrees (center-left)
//   const rotationAngle = -((360 / ITEM_COUNT) * selectedIndex) + 180;
...
//   return (
//     <div className="flex-1 relative bg-[#334] overflow-hidden h-screen">
//       {/* <BackgroundModel url="/model/test.glb" position="left" /> */}
//       <div
//         ref={carouselRef}
//         className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-1/2 w-[600px] h-[600px] rounded-full flex justify-center items-center z-0 select-none"
//       >
//         {/* The rotating container for all items */}
//         <div
...
//             const posX = (RADIUS + ITEM_OFFSET) * Math.cos((angle - 90) * (Math.PI / 180));
//             const posY = (RADIUS + ITEM_OFFSET) * Math.sin((angle - 90) * (Math.PI / 180));
            
//             // Calculate counter-rotation to keep text straight
//             const itemRotation = -rotationAngle;

//             return (
//               <div
//                 key={item.id}
//                 className={`absolute flex flex-col items-center justify-start p-2 cursor-pointer transition-all duration-300 ${
//                   index === selectedIndex ? "scale-110" : "scale-100"
//                 }`}
//                 style={{
//                   width: ITEM_WIDTH,
//                   height: ITEM_HEIGHT,
//                   left: `calc(50% + ${posX}px - ${ITEM_WIDTH / 2}px)`,
//                   top: `calc(50% + ${posY}px - ${ITEM_HEIGHT / 2}px)`,
//                   transform: `rotate(${itemRotation}deg)`, // Counter-rotate to keep content straight
//                 }}
//                 onClick={() => handleItemClick(index)}
//               >
//                 {/* Image/Model at top */}
//                 <div className="w-full h-4/5 flex items-center justify-center mb-1">
//                   {item.type === "model" ? (
//                     <SimpleModelViewer url={item.imageUrl} />
//                   ) : ( 
//                     <>
//                       {imageLoadError[item.id] ? (
//                         <div className="w-full h-full bg-gray-200 rounded border-2 border-dashed border-gray-400 flex items-center justify-center">
...
//                   )}
//                 </div>
//                 {/* Text at bottom - now stays straight */}
...
//     onClick={(e) => {
//       e.stopPropagation();
//       addToCart(item); // now actually adds to cart
//     }}
//     className="mt-1 bg-yellow-400 text-black text-xs px-3 py-1 rounded font-bold hover:bg-yellow-300"
//   >
//     ADD
//   </button>
//               </div>
...
//         <div className="absolute w-full h-full rounded-full border-4 border-white/20 pointer-events-none"></div>
//       </div>
//     </div>
//   );
// }
...
//   const selectedProduct = productImages[selectedIndex];
...
//     // Auto-hide the selected image after 3 seconds
//     setTimeout(() => {
...
//         : (selectedIndex - 1 + ITEM_COUNT) % ITEM_COUNT;
...
//   return (
//     <div className="flex-1 relative bg-[#334] overflow-hidden h-screen">
//       {/* Selected Product Display - Shows in center-left */}
//       {showSelectedImage && (
//         <div className="absolute left-1/4 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 transition-all duration-500 ease-in-out">
//           <div className="bg-white/90 backdrop-blur-sm rounded-lg p-6 shadow-2xl border-4 border-yellow-400">
//             <div className="w-48 h-48 flex items-center justify-center mb-4">
//               {selectedProduct.type === "model" ? (
//                 <SimpleModelViewer url={selectedProduct.imageUrl} />
//               ) : (
//                 <img
...
//                 className="bg-yellow-400 text-black px-6 py-2 rounded-lg font-bold hover:bg-yellow-300 transition-colors duration-200"
//               >
//                 ADD TO CART
//               </button>
//             </div>
...
//                 key={item.id}
//                 className={`absolute flex flex-col items-center justify-start p-2 cursor-pointer transition-all duration-300 ${
//                   index === selectedIndex ? "scale-110 ring-2 ring-yellow-400" : "scale-100"
//                 }`}
//                 style={{
//                   width: ITEM_WIDTH,
//                   height: ITEM_HEIGHT,
...
//                 <div className="w-full h-4/5 flex items-center justify-center mb-1">
//                   {item.type === "model" ? (
//                     <SimpleModelViewer url={"/images/transparen2t.png"} /> 
//                   ) : ( 
//                     <>
//                       {imageLoadError[item.id] ? (
//                         <div className="w-full h-full bg-gray-200 rounded border-2 border-dashed border-gray-400 flex items-center justify-center">
...
//                   className="mt-1 bg-yellow-400 text-black text-xs px-3 py-1 rounded font-bold hover:bg-yellow-300 transition-colors duration-200"
//                 >
//                   ADD
//                 </button>
//               </div>
...
// Example product list with categories
const productImages = [
  { id: 'red', name: 'red', category: 'Colors', imageUrl: '/images/transparent.png', type: 'image', price: '340' },
  { id: 'yellow', name: 'yellow', category: 'Colors', imageUrl: '/images/pixels.png', type: 'image', price: '240' },
  { id: 'blue', name: 'blue', category: 'Colors', imageUrl: '/images/transparen2t.png', type: 'image', price: '540' },
  { id: 'green', name: 'green', category: 'Colors', imageUrl: '/images/transparent.png', type: 'image', price: '340' },
  { id: 'brown', name: 'green', category: 'Colors', imageUrl: '/images/transparent.png', type: 'image', price: '340' },
  { id: 'emerald', name: 'Emerald', category: 'Colors', imageUrl: '/images/pixels.png', type: 'image', price: '240' },
  { id: 'Reddish', name: 'Reddish', category: 'Colors', imageUrl: '/images/transparen2t.png', type: 'image', price: '540' },
  { id: 'engineGuard', name: 'Engine Guard', category: 'Protection', imageUrl: '/images/engine.png', type: 'image', price: '640' },
  { id: 'mirror', name: 'Side Mirror', category: 'Protection', imageUrl: '/images/mirror.png', type: 'image', price: '140' },
  { id: 'luggage', name: 'Luggage Box', category: 'Protection', imageUrl: '/images/luggage.png', type: 'image', price: '540' },
  { id: 'engineGuard', name: 'Engine Guard', category: 'Protection', imageUrl: '/images/engine.png', type: 'image', price: '640' },
  { id: 'mirror', name: 'Side Mirror', category: 'Mirrors', imageUrl: '/images/mirror.png', type: 'image', price: '140' },
  { id: 'luggage', name: 'Luggage Box', category: 'Luggage', imageUrl: '/images/luggage.png', type: 'image', price: '540' },
];
...
export default function ProductPreview({ activeSub }: { activeSub: string }) {
  const { addToCart } = useCart();
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [imageLoadError, setImageLoadError] = useState<{ [key: string]: boolean }>({});
  const [showSelectedImage, setShowSelectedImage] = useState(false);
  ...
  // Filter items by submenu
  const filteredImages = productImages.filter((item) => item.category === activeSub);
  const ITEM_COUNT = filteredImages.length;

  const selectedProduct = filteredImages[selectedIndex] || filteredImages[0];
  const rotationAngle = -((360 / ITEM_COUNT) * selectedIndex) + 180;

  const handleItemClick = (index: number) => {
    setSelectedIndex(index);
    setShowSelectedImage(true);
    setTimeout(() => setShowSelectedImage(false), 3000);
  };
  ...
  // Reset selection when submenu changes
  useEffect(() => {
    setSelectedIndex(0);
  }, [activeSub]);
  ...
  return (
    <div className="flex-1 relative bg-[#334] overflow-hidden h-screen">
      ...
      {showSelectedImage && selectedProduct && (
        <div className="absolute left-1/4 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20">
          <div className="bg-white/90 backdrop-blur-sm rounded-lg p-6 shadow-2xl border-4 border-yellow-400">
            <div className="w-48 h-48 flex items-center justify-center mb-4">
              <img
                src={selectedProduct.imageUrl}
        ...
        >
          {filteredImages.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className={`absolute flex flex-col items-center p-2 cursor-pointer transition-all ${
                  index === selectedIndex ? 'scale-110 ring-2 ring-yellow-400' : 'scale-100'
                }`}
                style={{
                  width: ITEM_WIDTH,
              ...
              >
                ...
                <span className="text-white text-xs text-center w-full">{item.name}</span>
                <button
                  onClick={(e) => handleAddToCartClick(item, e)}
                  className="mt-1 bg-yellow-400 text-black text-xs px-3 py-1 rounded font-bold hover:bg-yellow-300"
                >
